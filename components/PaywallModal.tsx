  ...
  const handlePayment = async (plan: 'monthly' | 'lifetime') => {
      ...
      try {
          const priceId = plan === 'monthly' 
              ? 'price_YOUR_MONTHLY_PRICE_ID' // <-- SOSTITUISCI CON IL TUO PRICE ID DI STRIPE PER L'ABBONAMENTO MENSILE
              : 'price_YOUR_LIFETIME_PRICE_ID'; // <-- SOSTITUISCI CON IL TUO PRICE ID DI STRIPE PER IL PAGAMENTO UNA TANTUM
          
          const checkoutUrl = await getStripeCheckoutUrl(auth.currentUser.uid, priceId);
          
          if (checkoutUrl) {
              window.location.href = checkoutUrl;
          } else {
             // L'errore viene già gestito in stripeService, quindi qui non faccio nulla per non mostrare due alert
          }
      } catch (error) {
          console.error("Errore durante il checkout con Stripe:", error);
          alert("Si è verificato un errore durante la preparazione del pagamento. Riprova.");
      } finally {
        setLoading(null);
      }
  };
